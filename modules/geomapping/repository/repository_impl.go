...
func (r *repository) GetSensor(ctx context.Context, deviceId int) ([]map[string]interface{}, error) {
	...
	err := r.db.WithContext(ctx).Model(&models.Sensor{}).Where("device_id = ?", deviceId).Select("id, device_id, sensor_name, is_line, lat, lng, value, unit, update_date").Find(&sensors).Error
	if err != nil && errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil
	}
